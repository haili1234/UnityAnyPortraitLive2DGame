...
namespace AnyPortrait
{
	...
	public class apPhysicsVertParam
	{
		...
		public void RefreshLinkedVertex()
		{
			...
			//Vector2 localPos = _parentModVertWeight._vertex._pos;
			////public Vector2 _deltaPosLocal = Vector2.zero;
			////public float _distLocal = 0.0f;
			////public float _distWeight = 0.0f;

			////Local 기반 거리를 체크하고,
			////가까울 수록 weight를 증가시킨다.
			////다른 것들과 유효한지 체크한다.
			////굳이 유효한 것만 체크하는 건 아니다.
			////장력은 Enabled = false 인 것과도 행해지기 때문
			////IDW가 아니라
			////Linear Reverse Interpolation의 Normalize로 한다.

			//LinkedVertex linkVert = null;
			//float totalDist = 0.0f;
			//for (int i = 0; i < _linkedVertices.Count; i++)
			//{
			//	linkVert = _linkedVertices[i];

			//	Vector2 targetPos = linkVert._vertex._pos;

			//	linkVert._deltaPosLocalLinkToTarget = targetPos - localPos;
			//	linkVert._distLocal = Vector2.Distance(targetPos, localPos);
			//	linkVert._distWeight = 0.0f;
			//	totalDist += linkVert._distLocal;
			//}
			...
			//{
			//	linkVert = _linkedVertices[i];
			//	linkVert._distWeight = totalDist - linkVert._distLocal;//가까울 수록 Weight가 크다

			//	totalWeight += linkVert._distWeight;
			//}
			...
			//	{
			//		linkVert = _linkedVertices[i];
			//		linkVert._distWeight /= totalWeight;
			//	}
			//}
		}
