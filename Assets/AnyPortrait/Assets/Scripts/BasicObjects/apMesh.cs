...
namespace AnyPortrait
{
	...
	public class apMesh : MonoBehaviour
	{
		...
		//public int MatrixHashCode {  get { return _matrixHashCode; } }
		// [Vert Local Pos = Vert Pos - Offset]
		...
		/// <returns></returns>
		public bool MakeNewEdge(apVertex vert1, apVertex vert2, bool isAddCrossPoint)
		{
			//여기서부터 작업
			if (vert1 == vert2)
			{
		...
		public void MakeEdgesToPolygonAndIndexBuffer()
		{
			//Edge들을 연결해서
			//1. Polygon을 만든다.
			...
			for (int iEdge = 0; iEdge < _edges.Count; iEdge++)
			{
				...
				//단, 현재 Edge에 대해서 이미 어떤 최대 2개 이상의 폴리곤이 소유하고 있다면
				//이 Edge는 이미 처리가 되었다.
				//처리 실패
				//만약 1개가 이미 검색되었다면, 
				//최대 처리 개수는 1개이다.
				//List<apMeshPolygon> existPolygons = _polygons.FindAll(delegate (apMeshPolygon a)
				//{
				//	return a.IsEdgeContain(curEdge);
				//});

				//apMeshPolygon existPolygon = null;
				//if(existPolygons != null)
				//{
				//	if(existPolygons.Count == 1)
				//	{
				//		existPolygon = existPolygons[0];
				//	}
				//	else if(existPolygons.Count >= 2)
				//	{
				//		//이미 이 Edge는 더 처리가 불가능하다
				//		continue;
				//	}
				...
				List<MakePolygonResult> results = RecursiveMakePolygon(endVert, startVert, curEdge, 1, calculatedEdges, calculateVert);
				if (results == null)
				{
					//폴리곤을 만들지 못했다.
					continue;
		...
		public void AutoLinkEdges()
		{
			...
			for (int iLinkData = 0; iLinkData < linkData.Count; iLinkData++)
			{
				...
				List<MakePolygonResult> results = RecursiveMakePolygon(curLink._vert1, curLink._vert2, tmpEdge, 1, calculatedEdges, calculateVert, 8);
		...
		private List<MakePolygonResult> RecursiveMakePolygon(apVertex endVert, apVertex prevVert, apMeshEdge prevEdge, int curLevel, List<apMeshEdge> calculatedEdges, List<apVertex> calculatedVertex, int maxLevel = 8)
		{
			...
			for (int i = 0; i < _edges.Count; i++)
			{
				...
				else
				{
					...
					List<MakePolygonResult> nextResults = RecursiveMakePolygon(endVert, nextVert, nextEdge, curLevel++, newCalculatedEdges, newCalculatedVertex);
					if (nextResults != null)
					{
