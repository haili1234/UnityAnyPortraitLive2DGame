...
Shader "AnyPortrait/Editor/Clipped Colored Texture ToneColor (2X)"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(vertexOutput IN) : COLOR
			{
				...
				half a_0 = tex2D(_MainTex, IN.uv + float2(0.01f, 0.01f)).a;
				half a_1 = tex2D(_MainTex, IN.uv + float2(-0.01f, 0.01f)).a;
				half a_2 = tex2D(_MainTex, IN.uv + float2(0.01f, -0.01f)).a;
				half a_3 = tex2D(_MainTex, IN.uv + float2(-0.01f, -0.01f)).a;

				half a_4 = tex2D(_MainTex, IN.uv + float2(0.015f, 0)).a;
				half a_5 = tex2D(_MainTex, IN.uv + float2(0, 0.015f)).a;
				half a_6 = tex2D(_MainTex, IN.uv + float2(-0.015f, 0)).a;
				half a_7 = tex2D(_MainTex, IN.uv + float2(0, -0.015f)).a;
				...
				c.a *= _Color.a * maskTexture.a * outlineItp;
