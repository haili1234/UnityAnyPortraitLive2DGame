...
namespace AnyPortrait
{
	...
	public class apDialog_AddChildTransform : EditorWindow
	{
		...
		void OnGUI()
		{
			...
			GUILayout.Button(new GUIContent(strCategory, iconImageCategory), guiStyle_None, GUILayout.Height(20));//<투명 버튼
			...
			if (_target == TARGET.Mesh)
			{
				for (int i = 0; i < _meshes.Count; i++)
				{
					...
					GUILayout.Space(15);
					if (GUILayout.Button(new GUIContent(" " + mesh._name, iconImage_Mesh), curGUIStyle, GUILayout.Width(width - 35), GUILayout.Height(20)))
					{
			...
			else
			{
				for (int i = 0; i < _meshGroups.Count; i++)
				{
					...
					EditorGUILayout.BeginHorizontal(GUILayout.Width(width - 50));
					GUILayout.Space(15);
					if (GUILayout.Button(new GUIContent(" " + meshGroup._name, iconImage_MeshGroup), curGUIStyle, GUILayout.Width(width - 35), GUILayout.Height(20)))
					{
